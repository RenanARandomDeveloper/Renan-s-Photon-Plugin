...
public final class SummonFXEntityClient {
    ...
    private static final class ClientDispatcher {
        ...
        static void dispatch(List<Entity> entities, String effectName, Vec3 offset, Vec3 rotation, Vec3 scale,
            ...
            Minecraft.getInstance().execute(() -> {
                try {
                    ...
                    EntityEffectExecutor.AutoRotate autoRotate = toPhotonAutoRotate(autoRotateMode);

                    for (Entity entity : entities) {
                        var effect = new EntityEffectExecutor(fx, level, entity, autoRotate);
        ...
        private static EntityEffectExecutor.AutoRotate toPhotonAutoRotate(AutoRotateMode mode) {
            return switch (mode) {
                case FORWARD -> EntityEffectExecutor.AutoRotate.FORWARD;
                case LOOK -> EntityEffectExecutor.AutoRotate.LOOK;
                case XROT -> EntityEffectExecutor.AutoRotate.XROT;
